...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $angelegt = [];
  $fehler = [];
  $ins = db()->prepare('INSERT INTO nutzer (klasse_id, email, name, pass_hash) VALUES (?, ?, ?, ?)');
  foreach ($liste as $m) {
    ...
    try {
      $ins->execute([$klasseId, $email, $name, password_hash($passwort, PASSWORD_DEFAULT)]);
      $angelegt[] = ['email' => $email, 'name' => $name, 'passwort' => $passwort];
    } catch (PDOException) {
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
  ...
  db()->prepare("DELETE FROM nutzer WHERE id = ? AND rolle = 'schueler'")->execute([$id]);
  antwort(200, ['ok' => true]);
}

    ...
    <Layout titel="📊 Mein Lernstand">
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          <div key={label} className="rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="text-2xl">{icon}</div>
            <div className="text-2xl font-black text-slate-900 dark:text-slate-100">{wert}</div>
      ...
      {Object.keys(f.unterricht).length > 0 && (
        <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h2 className="mb-3 font-bold text-slate-900 dark:text-slate-100">🎓 Themen-Training</h2>
            ...
            {BEREICHE.map((b) => {
              ...
              return (
                <Link
                  key={b}
                  to={`/unterricht/${b}`}
                  className="flex items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 hover:border-sky-300 dark:border-slate-800 dark:bg-slate-800/60 dark:hover:border-sky-600"
                >
                  <span className="font-medium text-slate-800 dark:text-slate-200">{bereichName(b)}</span>
      ...
      {Object.values(lernpaare).some((l) => l.length > 0) && (
        <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h2 className="mb-1 font-bold text-slate-900 dark:text-slate-100">🧠 Themen-Quiz heute</h2>
                ...
                >
                  ...
                  <span
                    className={`shrink-0 rounded-full px-2.5 py-0.5 text-sm font-bold ${
                      stand.faellig > 0 ? 'bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300' : 'bg-green-100 text-green-800 dark:bg-green-950/40 dark:text-green-300'
                    }`}
                  >
      ...
      {rechnen && (() => {
        ...
        return (
          <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h2 className="mb-3 font-bold text-slate-900 dark:text-slate-100">🧮 Kaufmännisches Rechnen</h2>
                ...
                return (
                  <div key={k.id}>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-700 dark:text-slate-300">{k.titel}</span>
      ...
      <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h2 className="mb-3 font-bold text-slate-900 dark:text-slate-100">🏁 Deine Aufgabensammlungen</h2>
        {simulationen.length === 0 ? (
          ...
          <div className="space-y-2">
            {simulationen.map((s, i) => {
              ...
              return (
                <div key={i} className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 dark:border-slate-800 dark:bg-slate-800/60">
                  ...
                  <span
                    className={`ml-auto rounded-full px-2.5 py-0.5 text-sm font-bold ${
                      note <= 2 ? 'bg-green-100 text-green-800 dark:bg-green-950/40 dark:text-green-300' : note <= 4 ? 'bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300' : 'bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-300'
                    }`}
                  >
                    Note {note} {s.mitKI && '🤖'}
                  </span>
                  <span className="text-xs text-slate-400 dark:text-slate-500">{s.datum}</span>
                </div>
              )
            })}
          </div>
      ...
      {/* Problem-Aufgaben */}
      <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h2 className="mb-1 font-bold text-slate-900 dark:text-slate-100">🔁 Deine Problem-Aufgaben</h2>
        <p className="mb-3 text-sm text-slate-500 dark:text-slate-400">
          Diese Aufgaben hattest du am häufigsten falsch — nochmal üben lohnt sich.
        </p>
        {problemAufgaben.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">Bisher keine — weiter so! 💪</p>
        ) : (
          <div className="space-y-2">
            {problemAufgaben.map(({ aufgabe, v }) => (
              <Link
                ...
                className="block rounded-xl border border-red-100 bg-red-50/60 px-3 py-2 hover:border-red-300 dark:border-red-900 dark:bg-red-950/40 dark:hover:border-red-700"
              >
      ...
      <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h2 className="mb-1 font-bold text-slate-900 dark:text-slate-100">🗂️ Alle Themen im Überblick</h2>
        <p className="mb-3 text-sm text-slate-500 dark:text-slate-400">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h2 className="mb-3 font-bold text-slate-900 dark:text-slate-100">📚 Themen mit Luft nach oben</h2>
            ...
            {schwacheThemen.map(({ thema, q }) => {
              ...
              return (
                <Link
                  ...
                  className="block rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 hover:border-sky-300 dark:border-slate-800 dark:bg-slate-800/60 dark:hover:border-sky-600"
                >
                  <div className="flex items-baseline justify-between gap-3">
                    <span className="font-medium text-slate-800 dark:text-slate-200">{thema.name}</span>
                    <span className="shrink-0 text-sm font-semibold text-slate-500 dark:text-slate-400">{p} %</span>
                  </div>
